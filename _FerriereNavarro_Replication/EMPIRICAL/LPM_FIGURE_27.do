...
*--lhs variable: gdp or gov
local yvar "gov"

*--rhs variable: bp or news
local shock "news"
...
forvalues ih = 0 (1) $Hmax {					
	...
	reg dy `varreg' dg if yq>=$id & yq <= $fd, vce(robust)		
...
*--save
outsheet using "$outputdir${sep}LPM_FIGURE_27_`yvar'_`shock'.csv", comma noquote replace
