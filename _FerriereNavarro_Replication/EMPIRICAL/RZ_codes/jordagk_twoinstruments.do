...
global baseline_path "/Users/gastonnavarro/Dropbox/Axelle-Gaston/GovernmentSpending/REPLICATION/EMPIRICAL/RZ_codes"

cd "$baseline_path"


global FNsample = 1 // =0 for full sample (as in RZ), =1 for FN sample
...
forvalues i = 1/20 {; 
	...
	if $FNsample == 1{;
		ivreg2 f`i'cumuly (f`i'cumulg = $`shock'linshock bp) $`shock'linxlist if L`p'.`omit'==0 & F`i'.`omit'==0 & `omit'==0 & quarter>=1913 & quarter<=2006.75, robust bw(auto);
	};
	else{;
...
list h multlin1 seylin if h<=20;
if $FNsample == 1{;
	outsheet h multlin1 seylin using RZresults_FNsample.csv if h<=20,   comma replace ; // use this for FN sample (regression in line 256)
};
else{;
	outsheet h multlin1 seylin using RZresults_fullsample.csv if h<=20, comma replace ; // use this for full sample (regression in line 255)
}; 	
