module ModuleSTEADY
	...
	use ModuleMPC
    use Modulelpe
    use MODULEmpc_annual
    use MODULEmpc_persist
    use MODULEmpe
    use ModuleSAVE
    use Toolbox
...
contains
    ...
    btaH = 0.998825D0;  Dbta = 0.0575D0/dble(Nbeta-1);
...
!		print *, 'DB   = ', DB
    ...
    ! Compute quarterly MPC
	call COMPUTEmpc
    ! Compute lpe^tau and replicate Erosa et al.
    call COMPUTElpe
    ! Compute annual MPC
    call COMPUTEmpc_annual
    ! Compute MPE: replicate Golosov et al.
    call COMPUTEmpe

    ! For Section 4: compute MPC out of a persistent transfer
    call COMPUTEmpc_persist
