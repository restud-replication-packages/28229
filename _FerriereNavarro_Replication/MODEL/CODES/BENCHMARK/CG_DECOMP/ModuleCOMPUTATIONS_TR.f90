...
contains
	...
	implicit none
	real(8) :: yl(NS), yk(NS), rba, taxl, taxlss, GTOT, deltax, I0, Ixx(T_TR), taxrate(NS,Nbeta)
	integer :: tt, ib, ih, is
	...
	do tt = 1, T_TR
        ...
        Avgetax(tt) = sum(mu_TR(:,:,tt)*hpol_TR(:,:,tt,2)*taxrate)/sum(mu_TR(:,:,tt)*hpol_TR(:,:,tt,2))
