...
contains
	...
	open(1,  file = '../CG/OUTPUT/bbounds.txt',      status = 'unknown')
    read(1, *) bbounds
    close(1)
	...
	! guess = [rk, lbd]
	open(1,  file = '../CG/OUTPUT/guess.txt',      status = 'unknown')
    read(1, *) guess
    close(1)
	lbd = guess(2)
	...
	!---mu
     ...
     enddo
	open(1, file = '../CG/OUTPUT/muLOAD.txt',    status = 'unknown')
	read(1, *) mu
    close(1)
...
!			call SaveAggregates
    ...
    maxitV = 500; wwV = 0.25D0; tolV = 1e-9; maxitH = 40
    showV  = 10
