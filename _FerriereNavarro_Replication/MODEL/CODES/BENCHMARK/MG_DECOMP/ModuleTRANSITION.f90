...
contains
    ...
    if (index_decomp==1) then
        ...
        open(1,  file = '../MG/OUTPUT/wgeH_TR.txt',    status = 'unknown')
        open(2,  file = '../MG/OUTPUT/Pi_TR.txt',    status = 'unknown')
        read(1, *) wgeH_TR
        read(2, *) Pi_TR
        close(1); close(2);
        open(1,  file = '../MG/OUTPUT/rk_TR.txt',    status = 'unknown')
        read(1, *) rk_TR
        close(1)
        open(1,  file = '../MG/OUTPUT/div_TR.txt',    status = 'unknown')
        read(1, *) div_TR
        close(1)
        open(1, file = '../MG/OUTPUT/lbd_TR.txt', status = 'unknown')
        open(2, file = '../MG/OUTPUT/gma_TR.txt', status = 'unknown')
        read(1, *) lbd_TR
        read(2, *) gma_TR
        close(1); close(2);

        open(1, file = '../MG/OUTPUT/G_TR.txt', status = 'unknown')
        read(1, *) G_TR
        close(1)
