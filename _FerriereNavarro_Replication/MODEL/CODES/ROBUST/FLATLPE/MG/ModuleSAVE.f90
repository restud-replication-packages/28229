...
contains    
	...
	open(2,  file = 'OUTPUT/Kagg_TR.txt',   status = 'unknown')
    ...
    open(1, file = 'OUTPUT/index_tax_TR.txt',    status = 'unknown')
    do is = 1, NS
        write(1, '(*(f18.6))') ( index_tax_TR(is,ib), ib = 1, Nbeta)
    enddo
    close(1)
