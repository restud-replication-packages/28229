...
!			JACit = 0
		...
		elseif (errf(1) < 7e-5 .and. it > 1200) then
			...
			if (JACit==1) then
				...
				! Bjinvkp1 =  Compute_JACOBIAN(xkp1)
			else
				Bjinvkp1 = Update_JACOBIAN(xk,xkp1,fk,fkp1,Bjinvk)
				! Bjinvkp1 = Bjinvk
			endif
	...
	! print *, 'ED_ss = ', ED_ss
	! print *, 'max(abs(ED_ss)) = ', maxval(abs(ED_ss))
	
	! print *, 'Yagg_TR(1:10) - Ysup_TR(1:10) = ', Yagg_TR(1:10) - Ysup_TR(1:10)
	! print *, 'Aagg_TR(1:10) - (DB_TR(1:10)+Kdmn_TR(1:10)) = ', Aagg_TR(1:10) - (DB_TR(1:10)+Kdmn_TR(1:10))
	! print *, 'BC_TR(1:10) = ', BC_TR(1:10)
	! print *, 'Kagg_TR(1:10) - Kdmn_TR(1:10)	= ', Kagg_TR(1:10) - Kdmn_TR(1:10)
	
	! print *, 'Yagg_TR(1:10) = ', Yagg_TR(1:10)
	! print *, 'Ysup_TR(1:10) = ', Ysup_TR(1:10)
	
	! print *, 'Lagg, Kagg = ', Lagg, Kagg
	!  print *, 'Lagg_TR(1:10) = ', Lagg_TR(1:10)
	! print *, 'Kdmn_TR(1:10) = ', Kdmn_TR(1:10)
	
	! print *, 'Cagg, Aagg = ', Cagg, Aagg
	! print *, 'Cagg_TR(1:10) = ', Cagg_TR(1:10)
	! print *, 'Aagg_TR(1:10) = ', Aagg_TR(1:10)
	
	! print *, 'rk, wge, qk = ', rk, wge, qk
	! print *, 'rk_TR(1:10)  = ', rk_TR(1:10)
	! print *, 'wge_TR(1:10) = ', wge_TR(1:10)
	! print *, 'qk_TR(1:10)  = ', qk_TR(1:10)
	
	! print *, 'MC = ', MC
	! print *, 'MC_TR(1:10)  = ', MC_TR(1:10)
	
	! print *, 'divY_TR(1:10) = ', divY_TR(1:10)
	! print *, 'divK_TR(1:10) = ', divK_TR(1:10)
	
	! print *, 'DB = ', DB
	! print *, 'DB_TR(1:10) = ', DB_TR(1:10)
...
!    integer :: it, itsave, itax, tt
!    real(8) :: wwx, dbs, err_t(maxit), wwy
!
!    ibar = -1.0D0 + (1.0D0+rk)*Pibar
!
!    print *, 'wgeH, Piwbar, wge = ', wgeH, wge, Piwbar, FCW
!
!    !---initial point
!    xk(0*T_TR+1:1*T_TR) = rk
!    xk(1*T_TR+1:2*T_TR) = wge
!    xk(2*T_TR+1:3*T_TR) = wgeH
!    xk(3*T_TR+1:4*T_TR) = lbd  ! xk(2*T_TR+1:3*T_TR) = xlbd
!    xk(4*T_TR+1:5*T_TR) = 0d0   ! dividends
!    xk(5*T_TR+1:6*T_TR) = 1.0d0   ! qk
!
!    !---itearation
!    wwx = 0.90D0; tolf = 6e-7; itsave = 5; wwy = 0.9D0
!
!    do it = 1, maxit
!
!        !---evaluate guess
...
!        print *, '***  it, err = ', it, maxval(abs(fkp1(0*T_TR+1:1*T_TR))), maxval(abs(fkp1(1*T_TR+1:2*T_TR))), maxval(abs(fkp1(2*T_TR+1:3*T_TR))), maxval(abs(fkp1(3*T_TR+1:4*T_TR))), maxval(abs(fkp1(4*T_TR+1:5*T_TR)))
!        print *, '****************************'
!
!        if (errf(1) < 1e-2) then
!            wwx = 0.99D0
...
!            !--- update xk
!
!            xk(0*T_TR+1:1*T_TR) = wwx*rk_TR + (1d0-wwx)*rb_TR
!            xk(1*T_TR+1:2*T_TR) = wwx*wge_TR + (1d0-wwx)*wgeb_TR
!            xk(2*T_TR+1:3*T_TR) = wwx*wgeH_TR + (1d0-wwx)*wgeHb_TR
!            xk(3*T_TR+1:4*T_TR) = wwx*lbd_TR + (1d0-wwx)*lbdb_TR
!            xk(4*T_TR+1:5*T_TR) = wwy*div_TR + (1d0-wwy)*divb_TR
!            xk(5*T_TR+1:6*T_TR) = wwy*qk_TR + (1d0-wwy)*qkb_TR
!
!        endif
!
!        err_t(it) = errf(1)
