...
contains
	...
	lbd_TR        = xxin(1*T_TR+1:2*T_TR)   ! xlbd_TR       = xxin(2*T_TR+1:3*T_TR)
    div_TR        = xxin(2*T_TR+1:3*T_TR)
    qk_TR         = xxin(3*T_TR+1:4*T_TR)


    MC_TR    = (epsPi-1.0D0)/epsPi

    wge_TR = alpha*( (MC_TR*zbar)**(1.0D0/alpha) )*( ((1.0D0-alpha)/(rk_TR+(qk_TR*dlta)))**((1.0D0-alpha)/alpha) )

    wgeH_TR = ((epswPi-1d0)/epswPi)*wge_TR
    ...
    r_TR(1) = rk
    do tt = 2, T_TR
        r_TR(tt) = (qk_TR(tt)+rk_TR(tt))/qk_TR(tt-1) - 1d0
    enddo
	...
	ED_out(1*T_TR+1:2*T_TR) = BC_TR*1.0D0
    ED_out(2*T_TR+1:3*T_TR) = div_TR - divb_TR
    ED_out(3*T_TR+1:4*T_TR) = qk_TR - qkb_TR
