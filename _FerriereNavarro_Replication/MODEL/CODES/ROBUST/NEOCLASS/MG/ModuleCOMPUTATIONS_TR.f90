...
!            mm(:,ib)         = Pbeta(ib)*mm(:,ib)/sum(mm(:,ib))     
    ...
    ! To update wages, compute inflation [using the OLD]
    Pi_TR(T_TR) = Pibar
    do tt = T_TR-1,1,-1
        Pi_TR(tt) = Pibar* (((r_TR(tt+1) + 1.0D0)*Pi_TR(tt+1)/(1.0D0+ibar) )**(1d0/phiPI))
    enddo
    ! then marginal costs
...
!    MC_TR = MC_TR*(thetaPi/epsPi)
    ...
    ! Compute the new capital and investment
    do tt = 1, T_TR
        Dk_TR(tt) = Kagg_TR(tt+1) - (1.0D0-dlta)*Kagg_TR(tt)
    enddo
!    Ixx      = ( (Dk_TR/Kagg_TR(1:T_TR)) - phiK2 )/phiK0
!    Iagg_TR  = ( Ixx**(1.0D0/(1.0D0-phiK1)) )*Kagg_TR(1:T_TR)
!
!    qkb_TR = ( 1.0D0/(phiK0*(1.0D0-phiK1)) ) * ( (Iagg_TR/Kagg_TR(1:T_TR))**phiK1 )
!
!    PHI_TR   = phiK0*((Iagg_TR/Kagg_TR(1:T_TR))**(1.0D0-phiK1)) + phiK2
    ...
    !---Update r_TR
!    rkb_TR = ((1d0-alpha)/alpha)*wge_TR*(Lagg_TR/Kagg_TR(1:T_TR))-dlta*qkb_TR
    !rb_TR(1) = rk

    rkb_TR = MC_TR*(1d0-alpha)* ((Lagg_TR/Kagg_TR(1:T_TR))**alpha) - dlta* qk_TR


    !---update wge_TR
!    wgeb_TR = alpha*( (MC_TR*zbar)**(1.0D0/alpha) )*( ((1.0D0-alpha)/(rkb_TR+(qkb_TR*dlta)))**((1.0D0-alpha)/alpha) )
...
!    wgeHb_TR = wgeHb_TR*(thetawPi/epswPi)
    ...
    do tt = 1, T_TR
        !---divY_TR
        theta_TR(tt) = 0d0 !(thetaPi/2.0D0)*((Pi_TR(tt)-Pibar)**2.0D0)*Yagg_TR(tt)
