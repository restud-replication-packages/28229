...
contains
	...
	!-usual grid
!    avx  = amin + (amax-amin)*(gxa**(1.0D0/avecpar))
	!-adjust bmax to fit zero
!	iav  = vsearch_FET([0.0D0],avx)  ! avx(iav)<0<avx(iav+1)
!	iax  = iav(1) + 1              ! new grid will have zero at iax
!	dax  = (dble(iax-1)/dble(Na-1))**(1.0D0/avecpar)
!	amax = amin*(1.0D0-(1.0D0/dax) )
    avec = amin + (amax-amin)*(gxa**(1.0D0/avecpar))
	
	print *, 'amax = ', amax
	print *, 'iax = ', iax
	print *, 'avec(iax-3:iax+3) = ', avec(iax-3:iax+3)    
