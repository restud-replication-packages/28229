...
contains
	...
	INC  = yl + yk + S(is_in,1) + Tdx_TR(is_in,tt_in) + TF_TR(tt_in) - TAX
    ...
    real(8) 			:: T_out, aty_TR, aty_ss
    
    T_out = tauk_TR(tt_in)*yk
    if (yl > 0.0D0) then

        if (tt_in<=2) then
            aty_tr = lbd_TR(tt_in)*(yl**(1.0D0-gma))
            T_out = T_out + yl - aty_tr

        elseif (tt_in>50) then ! some may pay less taxes
            aty_tr = lbd_TR(tt_in)*(yl**(1.0D0-gma_TR(tt_in)))
            T_out = T_out + yl - aty_tr
        else
            aty_ss = lbd*(yl**(1.0D0-gma))
            aty_tr = lbd_TR(tt_in)*(yl**(1.0D0-gma_TR(tt_in)))

            if (aty_tr>=aty_ss) then    ! I don't pay less taxes than in steady-state
                T_out = T_out + yl - aty_ss
            else
                T_out = T_out + yl - aty_tr
            endif
