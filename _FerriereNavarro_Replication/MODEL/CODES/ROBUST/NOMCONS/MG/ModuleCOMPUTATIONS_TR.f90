...
!            mm(:,ib)         = Pbeta(ib)*mm(:,ib)/sum(mm(:,ib))     
	...
	implicit none
	real(8) :: yl(NS), yk(NS), rba, taxl, taxlss, GTOT, deltax, I0, Iss, aty_tr, aty_ss, Ixx(T_TR), I0_init
	integer :: tt, ib, ih, is
			...
			do is = 1, NS
				...
				if (yl(is)>0.0D0) then
                    ...
                    if (tt<=2) then
                        ...
                        aty_tr = lbd_TR(tt)*(yl(is)**(1.0D0-gma))

                        taxl            = yl(is) - aty_tr
                        RevL_TR(tt)     = RevL_TR(tt) + taxl*hpol_TR(is,ib,tt,ih)*mu_TR(is,ib,tt)
                        I0_init  = I0_init + ((yl(is)**(1.0D0-gma)) )*hpol_TR(is,ib,tt,ih)*mu_TR(is,ib,tt)
