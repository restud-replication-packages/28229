...
contains
    ...
    real(8) :: GTOT, yl(NS), yk(NS), rba, hh(NS), hg0(NS), EMPv2, taxrate(NS,Nbeta)
    integer :: is, ib, ih
	...
	do ih = 1, Nh
		...
		Hagg = Hagg + sum(hvec(ih)*hpol(:,ib,ih)*mu(:,ib))
	...
	do ih = 1, Nh
		do is = 1, NS
			...
			if (yl(is) > 0.0D0 ) then
                ...
                taxrate(is,ib) = ( yl(is) - lbd*(yl(is)**(1.0D0-gma)) )/yl(is)
    ...
    print *, 'avge labor tax = ', sum(mu*hpol(:,:,2)*taxrate)/sum(mu(:,:)*hpol(:,:,2))
